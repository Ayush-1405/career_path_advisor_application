...
class EditCareerPathScreen extends ConsumerStatefulWidget {
  final CareerPath path;
  
  const EditCareerPathScreen({super.key, required this.path});

  @override
  ConsumerState<EditCareerPathScreen> createState() => _EditCareerPathScreenState();
}

class _EditCareerPathScreenState extends ConsumerState<EditCareerPathScreen> {
  ...
  void initState() {
    ...
    _popularityController = TextEditingController(text: p.popularity.toString());
    _skillsController = TextEditingController(text: p.requiredSkills.join(', '));
  ...
  Future<void> _handleSave() async {
    ...
    try {
      await ref.read(apiServiceProvider).updateCareerPath(widget.path.id, data);

      if (mounted) {
        context.pop(true); // Return true to indicate refresh needed
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Career path updated successfully'),
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            Expanded(child: _buildField('Level', _levelController)),
                          ],
                        ),
                      ],
                    ),
                    ...
                    _buildSectionCard(
                      ...
                      children: [
                        Row(
                          children: [
                            Expanded(child: _buildField('Avg. Salary', _salaryController)),
                            const SizedBox(width: 16),
                            Expanded(child: _buildField('Growth Rate', _growthController)),
                          ],
                        ),
                        const SizedBox(height: 20),
                        _buildField('Popularity (0-100)', _popularityController, isNumber: true),
                      ],
                    ),
                    const SizedBox(height: 24),
                    _buildSectionCard(
                      ...
                      children: [
                        _buildField('Image URL', _imageController),
                        const SizedBox(height: 20),
                        _buildField('Description', _descriptionController, isMultiline: true),
                      ],
                    ),
                    const SizedBox(height: 24),
                    _buildSectionCard(
  ...
  Widget _buildField(String label, TextEditingController controller, {bool isMultiline = false, bool isNumber = false, String? hint}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
